...
exports.seed = ({ itemsData, messagesData, categoriesData, usersData }) => {
  return db
    ...
    .then(()=>{
        const insertCategories = format(
            ...
            categoriesData.map(({ category_name, description, img_url }) => [category_name, description, img_url])
          );
    

        const insertUsers = format(
            'INSERT INTO users (username,name, area, email, rating, avatar_url) VALUES %L;',
            usersData.map(({ username,name, area, email, rating, avatar_url}) => [username,name, area, email, rating, avatar_url])
          );
    ...
    .then(()=>{
        ...
        const insertitems = format(
            ...
            itemsData.map(({ item_name, category_id, user_id, description, image_url,collection_point, date_of_expire, date_listed, reserved_for_id, reserve_status, collection_state}) => [item_name, category_id, user_id, description, image_url,collection_point, date_of_expire, date_listed, reserved_for_id, reserve_status, collection_state])
          );
    ...
    .then(()=>{
        ...
        const insertmessages = format(
            'INSERT INTO messages (sender_id, reciever_id, body, sent_at) VALUES %L;',
            messagesData.map(({ sender_id, reciever_id, body, sent_at}) => [sender_id, reciever_id, body, sent_at])
          );
